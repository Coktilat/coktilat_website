<?php
add_action( 'widgets_init', 'kidz_categort_posts_widget' );
function kidz_categort_posts_widget() {
	register_widget( 'kidz_categort_posts' );
}
class kidz_categort_posts extends WP_Widget {

	public function __construct(){
		$widget_ops 	= array( 'classname' => 'categort-posts' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'categort-posts-widget' );
		parent::__construct( 'categort-posts-widget', THEME_NAME .' - '.__( "Category Posts" , 'kidz' ) , $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<ul>
					<?php kidz_last_posts_cat($no_of_posts , $thumb , $cats_id)?>
				</ul>
		<div class="clear"></div>
	<?php
		echo $after_widget;
	...
	public function form( $instance ) {
		$defaults = array( 'title' =>__( 'Category Posts' , 'kidz'), 'no_of_posts' => '5' , 'cats_id' => '1' , 'thumb' => 'true' );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumb' ); ?>"><?php _e( 'Display Thumbnails :' , 'kidz') ?></label>
